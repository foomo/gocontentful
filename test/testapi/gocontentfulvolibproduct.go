...
func (cc *ContentfulClient) cacheProductByID(ctx context.Context, id string, entryPayload *contentful.Entry, entryDelete bool) error {
	...
	// It was deleted
	if len(col.Items) == 0 || entryDelete {
	...
	for _, loc := range cc.locales {
		...
		if okChildren {
			for _, child := range children {
				...
				if !found {
					cc.Cache.parentMap[child.Sys.ID] = append(cc.Cache.parentMap[child.Sys.ID], EntryReference{ContentType: product.Sys.ContentType.Sys.ID, ID: product.Sys.ID, VO: product})
				}
			}
		}
	}
