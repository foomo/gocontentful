...
func (genericEntry *GenericEntry) fieldLocale(locale []Locale) (Locale, error) {
	var loc Locale
	if len(locale) != 0 {
		loc = locale[0]
		if _, ok := localeFallback[loc]; !ok {
...
func (genericEntry *GenericEntry) FieldAsString(fieldName string, locale ...Locale) (string, error) {
	loc, err := genericEntry.fieldLocale(locale)
	if err != nil {
		return "", err
	}
	...
	rawFieldLocale, ok := rawField.(map[string]string)
	if !ok {
		return "", ErrNotSet
	}
	if value, ok := rawFieldLocale[string(loc)]; ok {
		return value, nil
	} else if value, ok = rawFieldLocale[string(DefaultLocale)]; ok {
		return value, nil
	}
	return "", ErrNotSet
}
...
func (genericEntry *GenericEntry) FieldAsFloat64(fieldName string, locale ...Locale) (float64, error) {
	...
	rawFieldLocale, ok := rawField.(map[string]float64)
	if !ok {
		return 0, ErrNotSet
	}
	if value, ok := rawFieldLocale[string(loc)]; ok {
...
func (genericEntry *GenericEntry) FieldAsReference(fieldName string, locale ...Locale) (*EntryReference, error) {
	...
	rawFieldLocale, ok := rawField.(map[string]ContentTypeSys)
	if !ok {
		return nil, ErrNotSet
	}
...
func (genericEntry *GenericEntry) FieldAsBool(fieldName string, locale ...Locale) (bool, error) {
	...
	rawFieldLocale, ok := rawField.(map[string]bool)
	if !ok {
		return false, ErrNotSet
	}
	if value, ok := rawFieldLocale[string(loc)]; ok {
...
func (genericEntry *GenericEntry) FieldAsAny(fieldName string, locale ...Locale) (any, error) {
	...
	var rawFieldLocale map[string]any
	if err := MapStructure(rawField, &rawFieldLocale); err != nil {
		return nil, err
	}
	if value, ok := rawFieldLocale[string(loc)]; ok {
		return value, nil
...
func (genericEntry *GenericEntry) SetField(fieldName string, fieldValue any, locale ...Locale) error {
	loc, err := genericEntry.fieldLocale(locale)
	if err != nil {
		return err
	}
	...
	m.SetMapIndex(reflect.ValueOf(string(loc)), reflect.ValueOf(fieldValue))
	genericEntry.RawFields[fieldName] = m.Interface()
